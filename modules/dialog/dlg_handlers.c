	...
	struct sip_msg *msg)
{
	...
	if (dlg->legs[leg].last_gen_cseq) {
		...
		LM_NOTICE("storing cseq [%d] in slot [%d] for leg [%d] from last_gen_cseq\n", map->gen, msg_cseq, leg);
	} else {
		map->gen = msg_cseq;
		LM_NOTICE("storing cseq [%d] in slot [%d] for leg [%d] from msg\n", map->gen, msg_cseq, leg);
	}
	...
	dlg->legs_no[DLG_LEG_200OK] != leg) {
		...
		if (dlg->flags & DLG_FLAG_CSEQ_ENFORCE) {
			...
			if (str2int( &(get_cseq(rpl)->number), &cseq_no) < 0) {
				...
				LM_NOTICE("========= last_gen_cseq = cseq_no [%d] for method id [%d]\n", cseq_no, get_cseq(rpl)->method_id);
		...
		struct tmcb_params *ps)
{
	...
	if (get_cseq(req)->method_id == METHOD_INVITE) {
		LM_NOTICE("dlg_leg_push_cseq_map(dlg, [%d], INVITE)\n", DLG_CALLER_LEG);
													...
													struct tmcb_params *param)
{
	...
	LM_NOTICE("update_msg_cseq(param->rpl,%.*s,0)\n", ((dlg_cseq_wrapper *)*param->param)->cseq.len, ((dlg_cseq_wrapper *)*param->param)->cseq.s);
		...
		struct sip_msg *msg)
{
	...
	for (map = dlg->legs[leg].cseq_maps; map; map = map->next) {
		...
		LM_NOTICE("found cseq [%d] in map for leg [%d]\n", msg_cseq, leg);
		return msg_cseq;
	}
	LM_NOTICE("no cseq found in map for leg[%d]\n", leg);
	return 0;
}
...
void dlg_onroute(struct sip_msg* req, str *route_params, void *param)
{
	...
	if (dlg==0) {
		...
		LM_NOTICE("get_dlg => dst_leg=[%d]\n", dst_leg);
	}
	else {
		LM_NOTICE("match_dialog => dst_leg=[%d]\n", dst_leg);
	}
	...
	if (new_state==DLG_STATE_DELETED && old_state==DLG_STATE_DELETED) {
		...
		if (dlg->legs[dst_leg].last_gen_cseq) {
			...
			LM_NOTICE("last_gen_cseq is [%d]\n", dlg->legs[dst_leg].last_gen_cseq);
			update_val = ++(dlg->legs[dst_leg].last_gen_cseq);
			LM_NOTICE("========= incrementing last_gen_cseq to [%d]\n", dlg->legs[dst_leg].last_gen_cseq);
			dlg_unlock (d_table,d_entry);

			LM_NOTICE("update_msg_cseq(BYE,NULL,%d)\n", update_val);
	...
	&& (new_state==DLG_STATE_CONFIRMED || new_state==DLG_STATE_CONFIRMED_NA) ) {
		...
		if ( event!=DLG_EVENT_REQACK ) {
			...
			dlg->legs[dst_leg].cseq_maps) {
				LM_NOTICE("dlg_leg_get_cseq(dlg, [%d], req)\n", dst_leg);
				update_val = dlg_leg_get_cseq(dlg, dst_leg, req);
				if (update_val == 0) {
					LM_NOTICE("dlg->legs[%d].last_gen_cseq=[%d]\n", dst_leg, dlg->legs[dst_leg].last_gen_cseq);
					update_val = dlg->legs[dst_leg].last_gen_cseq;
				}
				else {
					LM_NOTICE("update_val=[%d]\n", update_val);
				}
				dlg_unlock( d_table, d_entry );
